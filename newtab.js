...
function render(node, target) {
    ...
    if (url) a.href = url; else a.tabIndex = 0;
    ...
    if (node.action) {
        ...
        if (urlStart === 'chrome' || urlStart === 'file:/') a.onclick = function () {
            openLink(node, newtab);
            return false;
        };

    } else if (!node.children && !node.type) a.style.pointerEvents = 'none';
    ...
    } else if (node.type) addAppHandlers(node, a);
...
function renderAll(nodes, target, toplevel) {
    ...
    if (ul.childNodes.length === 0) render({id: 'empty', title: '< Empty >'}, ul);
    if (toplevel) target.appendChild(ul); else {
...
function renderColumn(index, target) {
    ...
    if (ids.length == 1 && ids[0] != 'weather' && !getConfig('show_root')) getChildrenFunction({id: ids[0]})(function (result) {
        ...
        var callback = function (result) {
            ...
            i++;
            if (i < ids.length) getSubTree(ids[i], callback); else {
                // render node list
...
function addFolderHandlers(node, a) {
    ...
    if (!getConfig('lock')) {
        ...
        items.push({
            label: 'Create new column', action: function () {
                addColumn([node.id]);
            }
        });
        ...
        if (coords[node.id]) {
            ...
            if (pos.y > 0) items.push({
                label: 'Move folder up', action: function () {
                    addRow(node.id, pos.x, pos.y - 1);
                }
            });
            if (pos.y < columns[pos.x].length - 1) items.push({
                label: 'Move folder down', action: function () {
                    addRow(node.id, pos.x, pos.y + 2);
                }
            });
            if (pos.x > 0) items.push({
                label: 'Move folder left', action: function () {
                    addRow(node.id, pos.x - 1);
                }
            });
            if (pos.x < columns.length - 1) items.push({
                label: 'Move folder right', action: function () {
                    addRow(node.id, pos.x + 1);
                }
            });
            if (root.indexOf(node.id) < 0) items.push({
                label: 'Remove folder', action: function () {
...
function addAppHandlers(node, a) {
    ...
    a.oncontextmenu = function (event) {
        ...
        if (node.appLaunchUrl) {
            menuItems.push({
                label: 'Open in new tab', action: function () {
                    openLink(node, 1);
                }
            });
        }
        ...
        if (node.homepageUrl) {
            menuItems.push({
                label: 'Open in Web Store', action: function () {
        ...
        if (node.mayDisable) {
            menuItems.push({
                label: 'Uninstall', action: function () {
                    if (confirm('Uninstall "' + a.innerText + '"?')) chrome.management.uninstall(node.id, chrome.tabs.reload);
                }
            });
        }
    ...
    li.ondrop = function (event) {
        ...
        if (index == -1 || oldIndex == -1) return false;

        if (isAbove(event.pageY, target)) index++;
...
function addColumnHandlers(index, ul) {
    ...
    if (!getConfig('lock') && columns.length > 1) {
        ...
        if (index > 0) items.push({
            label: 'Move column left', action: function () {
                addColumn(ids, index - 1);
            }
        });
        if (index < columns.length - 1) items.push({
            label: 'Move column right', action: function () {
                addColumn(ids, index + 2);
            }
        });
        items.push({
            label: 'Remove column', action: function () {
                removeColumn(index);
            }
        });
        if (ids.length == 1) {
    ...
    if (items.length > 0) ul.oncontextmenu = function (event) {
...
function getMenuItems(node) {
    var items = [];
    if (node.id == 'weather') items.push({
        label: 'Update weather', action: function () {
            refreshWeather();
        }
    }); else items.push({
        label: 'Open all links in folder', action: function () {
            openLinks(node);
        }
    });
    if (node.id == 'closed') items.push({
        label: 'Clear browsing data', action: function () {
            openLink({url: 'chrome://settings/clearBrowserData'}, 1);
        }
    });
    if (node.id == 'apps') items.push({
        label: 'Manage apps', action: function () {
            openLink({url: 'chrome://apps'}, 1);
        }
    });
    if (node.id == 'devices') items.push({
        label: 'History', action: function () {
            openLink({url: 'chrome://history'}, 1);
        }
    });
    if (Number(node.id)) items.push({
        label: 'Edit bookmarks', action: function () {
...
function renderMenu(items, x, y) {
    ...
    for (var i = 0; i < items.length; i++) {
        ...
        if (items[i]) {
            ...
            li.appendChild(a);
        } else if (i > 0 && i < items.length - 1) li.appendChild(document.createElement('hr')); else continue;
...
function enableDragDrop() {
    ...
    main.ondrop = function (event) {
        ...
        if (dragIds.length == 1 && y != null) addRow(dragIds[0], x, y); else {
...
function getDropTarget(event) {
    ...
    if (target && (target.tagName == 'A' || target.parentNode.tagName == 'A') && dragIds.length == 1) {
        ...
        if (target && target.tagName == 'LI' && columns[getDropX(target, event)].length == 1) target = target.parentNode;
        // target should be LI or UL by here...
    } else while (target && target.className != 'column') target = target.parentNode;// target column
    ...
    switch (node.id) {
        case 'top':
            return function (callback) {
                if (chrome.topSites) chrome.topSites.get(function (result) {
        ...
        // 		};
        default:
            if (node.children) return function (callback) {
                callback(node.children);
            }; else return function (callback) {
                chrome.bookmarks.getSubTree(node.id, function (result) {
                    if (result) callback(result[0].children); else {
    ...
    switch (id) {
        ...
        default:
            chrome.bookmarks.getSubTree(id, function (result) {
                if (result) callback(result); else {
                    // remove missing bookmark locations
...
function setClass(target, node, isopen) {
    ...
    if (isopen) target.classList.add('open'); else target.classList.remove('open');
...
function getIcon(node) {
    ...
    } else if (node.icon) url = node.icon; else if (node.url || node.appLaunchUrl) {
...
function toggle(node, a) {
    ...
    if (isopen) {
        ...
        // open folder
        if (a.nextSibling) animate(node, a, isopen); else getChildrenFunction(node)(function (result) {
...
function animate(node, a, isopen) {
    ...
    a.animationHandle = setTimeout(function () {
        a.animationHandle = null;
        if (isopen) a.parentNode.removeChild(wrap); else {
            wrap.className = null;
...
function openLinks(node) {
    chrome.tabs.getCurrent(function (tab) {
        getChildrenFunction(node)(function (result) {
            for (var i = 0; i < result.length; i++) openLink(result[i], 2);
        });
    });
}
...
function openLink(node, newtab) {
    ...
    if (url) {
        chrome.tabs.getCurrent(function (tab) {
            if (newtab) chrome.tabs.create({
                url: url, active: (newtab == 1), openerTabId: tab.id
...
function verifyColumns() {
    ...
    for (var i = 0; i < missing.length; i++) {
        if (getConfig('show_' + missing[i]) != false) column.push(missing[i]);
    }
...
function loadColumns() {
    ...
    if (root) {
        ...
        chrome.bookmarks.getTree(function (result) {
            ...
            for (var i = 0; i < nodes.length; i++) root.push(nodes[i].id);
...
function saveColumns() {
    ...
    for (var x = 0; ; x++) {
        for (var y = 0; ; y++) {
            var id = localStorage.getItem('column.' + x + '.' + y);
            if (id) localStorage.removeItem('column.' + x + '.' + y); else break;
        }
        if (y === 0) break;
    }
    verifyColumns();
...
function getApps(callback) {
    chrome.management.getAll(function (result) {
        result = result.filter(function (a) {
            return a.enabled && a.type !== 'extension' && a.type !== 'theme' && a.isApp !== false && a.id !== 'nmmhkkegccagdldgiimedpiccmgmieda';// hide "Google Wallet Service"
        });
        ...
        result.sort(function (a, b) {
            ...
            if (diff) return diff; else if (a.name < b.name) return -1; else if (a.name > b.name) return 1; else return 0;
        });

        appsOrder = [];
        for (var i = 0; i < result.length; i++) appsOrder.push(result[i].id);
...
function getClosed(callback) {
    ...
    chrome.sessions.getRecentlyClosed({maxResults: maxResults}, function (sessions) {
        ...
        for (var i = 0; i < sessions.length && i < maxResults; i++) {
            (function (session) {
                if (session.window && session.window.tabs.length == 1) session.tab = session.window.tabs[0];
            ...
            (function (device) {
                ...
                for (var j = 0; j < device.sessions.length; j++) {
                    ...
                    for (var k = 0; k < tabs.length; k++) {
                        children.push({
                            title: tabs[k].title, url: tabs[k].url
                        });
                    }
                }
                nodes.push({
                    id: 'device.' + device.deviceName, title: device.deviceName, children: children
                });
            })(devices[i]);
        }
        callback(nodes);
...
function refreshClosed() {
    ...
    getChildrenFunction({id: 'closed'})(function (result) {
        for (var i = 0; i < targets.length; i++) renderAll(result, targets[i]);
    });
}
    ...
    request.onload = function (event) {
        try {
            ...
            for (var i = 0; i < forecast.length && i < 5; i++) {
                nodes.push({
                    title: forecast[i].day + ' ' + forecast[i].high + '°, ' + forecast[i].low + '° ' + forecast[i].text,
...
function refreshWeather(data, url) {
    // cache data
    if (data) localStorage.setItem('weather.cache', JSON.stringify({
        data: data, url: url, date: new Date()
    })); else localStorage.removeItem('weather.cache');
    // render
    var targets = document.getElementsByClassName('weather');
    for (var i = 0; i < targets.length; i++) {
...
var config = {
    ...
    shadow_color: '#57b0ff',
    font_color_dark: '#bfdfff',
    background_color_dark: '#101827',
    highlight_color_dark: '#000000',
    highlight_font_color_dark: '#80ecff',
    shadow_color_dark: '#0080ff',
...
function getConfig(key) {
    ...
    if (value != null) {
        ...
        if (isDarkMode()) return (theme_dark.hasOwnProperty(key) ? theme_dark[key] : config[key]); else return (theme.hasOwnProperty(key) ? theme[key] : config[key]);
    }
}
...
function setConfig(key, value) {
    ...
    if (value != null) localStorage.setItem('options.' + key, typeof config[key] === 'number' ? Number(value) : value); else {
        localStorage.removeItem('options.' + key);
        if (isDarkMode()) value = (theme_dark.hasOwnProperty(key) ? theme_dark[key] : config[key]); else value = (theme.hasOwnProperty(key) ? theme[key] : config[key]);
    }
    // special case settings
    if (key == 'lock' || key == 'newtab' || key == 'show_root' || key.substring(0, 6) == 'number') loadColumns(); else if (key == 'theme') {
        theme = themes[value];
        for (var i in config) {
            if (i != key) {
                if (!isDarkMode()) onChange(i);
                showConfig(i);
            }
        }
    } else if (key == 'theme_dark') {
        theme_dark = themes_dark[value];
        for (var i in config) {
            if (i != key) {
                if (isDarkMode()) onChange(i);
                showConfig(i);
        ...
        if (key == 'weather_location') setConfig('weather_location_id', null); else refreshWeather();
...
function getStyle(key, value) {
    switch (key) {
        ...
        case 'spacing':
            return '#main a { line-height: ' + scale(value, 2, 5.6, .8) + '; ' + 'padding-left: ' + scale(value, .8, 2, .4) + 'em; ' + 'padding-right: ' + scale(value, .8, 2, .4) + 'em; }';
        case 'width':
            return '#main { width: ' + (getConfig('auto_scale') ? scale(value, 80, 100, 20) + '%' : scale(value, 1000, 3000, 400) + 'px') + '; }';
        case 'h_pos':
            var margin = 100 - scale(getConfig('width'), 80, 100, 20);
            return '#main { left: ' + scale(value, 0, margin / 2, -margin / 2) + '%; }';
        case 'v_margin':
            return '#main { margin-top: ' + (getConfig('auto_scale') ? scale(value, 5, 20) + '%' : scale(value, 80, 600) + 'px') + '; }';
        case 'hide_options':
...
function onChange(key, value) {
    if (value == null) value = getConfig(key);

    if (value != config[key]) {
        var css = getStyle(key, value);
        if (css) {
            var style;
            if (styles.hasOwnProperty(key)) style = styles[key]; else {
    ...
    if (key == 'width') onChange('h_pos'); else if (key == 'shadow_blur') onChange('shadow_color'); else if (key == 'auto_scale') {
        onChange('width');
        onChange('v_margin');
    }
...
function loadSettings() {
    ...
    theme_dark = themes_dark[getConfig('theme_dark')] || {};
    // load settings
    for (var key in config) if (key === 'background_image_file') setTimeout(function () {
...
function initConfig(key) {
    ...
    input.onchange = function (event) {
        ...
        } else setConfig(key, input.type == 'checkbox' ? Number(input.checked) : input.value);
    };
...
function initSettings() {
        ...
        a.onclick = function (e) {
            ...
            if (index === nav.children.length - 1) {
                ...
                for (var key in config) {
                    ...
                    if (css && css.length < 1000 && key != 'css') allcss.value += css + '\n';
                }
            }
            // import/export
    ...
    chrome.bookmarks.getTree(function (result) {
        ...
        // show settings
        for (var key in config) initConfig(key);

        loadSettings();

        // load themes
        var select_dark = document.getElementById('options_theme_dark');
        ...
        if (select_dark.childNodes.length === 0) {
            for (var i in themes_dark) {
                var option = document.createElement('option');
                option.innerText = i;
                if (i == getConfig('theme_dark')) option.selected = 'selected';
                select_dark.appendChild(option);
            }
        }
